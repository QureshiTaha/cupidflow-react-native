...
const ReelItem = React.memo(
  ({ item, shouldPlay, currentIndex, index, currentUser, isLastItem }: any) => {
    ...
    const sendComment = useCallback(async () => {
      if (editingCommentId) return; // ✅ prevent add while editing
    ...
    const deleteComment = useCallback(async () => {
      ...
      try {
        const response = await axios.post(
          ...
          { commentId: String(deleteCommentID) },
          { headers: { "Content-Type": "application/json" } },
        );

        if (response.data.status) {
          setVideoComments((prev) =>
            prev.filter((c) => String(c.commentId) !== String(deleteCommentID)),
          );
          toast.show("Comment deleted successfully", { type: "success" });
        } else {
          toast.show(response.data.message || "Failed to delete comment", {
            type: "danger",
          });
        }
      } catch (err) {
        ...
        if (err.response) {
          ...
          toast.show("An error occurred while deleting comment", {
            type: "danger",
          });
        }
      } finally {
    ...
    // Add this function after the deleteComment function:
    const editComment = useCallback(async () => {
      if (!editingCommentId || !editCommentText.trim()) {
        console.log("❌ Edit comment: Missing ID or text");
        return;
      }
      ...
      console.log("🔄 Editing comment:", {
        ...
        currentText: videoComments.find((c) => c.commentId === editingCommentId)
          ?.commentText,
      });

      try {
        const response = await axios.put(
          `${BASE_URL}/api/v1/reels/edit-comment`,
          {
            commentId: editingCommentId,
        ...
        if (response.data.status) {
          // Update the comment in the videoComments array
          setVideoComments((prev) =>
            prev.map((comment) =>
          ...
          toast.show(response.data.message || "Failed to update comment", {
            type: "danger",
          });
        }
      } catch (err) {
        ...
        if (err.response) {
          ...
          toast.show("An error occurred while updating comment", {
            type: "danger",
          });
        }
      }
    }, [editingCommentId, editCommentText, videoComments]);
        ...
        <View style={styles.overlay}>
          <View style={styles.iconContainer}>
            ...
            <Pressable
              style={styles.iconWrapper}
              onPress={() => {
                // Reset edit state when opening comments
                setEditingCommentId(null);
                setEditCommentText("");
                showFetchedComments({ pageNumber: 1 });
              }}
            >
          ...
          <View style={styles.profileContainer}>
            ...
            <View style={styles.profileInfo}>
              <Text
                style={[styles.username, { color: "#fff", width: "65%" }]}
                numberOfLines={1}
                onPress={() =>
                  currentUserID === item.userID
                    ? router.push("../pages/profile")
            ...
            <View style={styles.modalOverlay}>
              <TouchableWithoutFeedback>
                <View style={styles.commentModal}>
                  ...
                  {/* {renderComments()} */}
                  {/* Comment Input */}
                  <View style={[styles.commentInputContainer, { gap: 8 }]}>
                    <Image
                      source={
                    ...
                    <TextInput
                      ...
                      value={editingCommentId ? editCommentText : newComment}
                      onChangeText={
                        editingCommentId ? setEditCommentText : setNewComment
                      }
                        ...
                        renderItem={({ item }) => {
                          return (
                            <TouchableOpacity
                              ...
                              onLongPress={() => {
                                // Long press for edit (only if user owns the comment)
                                if (currentUserID === item.userID) {
                                  setEditingCommentId(item.commentId);
                                  setEditCommentText(item.commentText);
                                }
                              }}
            ...
            <View style={styles.deletedOverlay}>
              <TouchableWithoutFeedback onPress={() => {}}>
                <View style={styles.deleteCommentModal}>
                  <Text style={styles.deleteCommentHeader}>Delete Comment</Text>
                  <Text style={styles.deleteCommentDescription}>
            ...
            <View style={styles.modalContainer}>
              <View style={styles.modalOptionRow}>
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.modalText, { color: themeColors.color1 }]}
                  >
                    Delete
                  </Text>
                </TouchableOpacity>
