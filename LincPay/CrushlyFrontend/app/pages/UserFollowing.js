...
export default function Following() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      fetchFollowingData(JSON.parse(User), 1, true);
    };
    getUser();
  }, []);

  const fetchFollowingData = async (user, pageNum = 1, initialLoad = false) => {
    if (initialLoad) {
      setIsLoading(true);
    } else if (pageNum > 1) {
      setLoadingMore(true);
    ...
    try {
      const response = await axios.get(
        ...
        }/api/v1/follow/getFollowingList/${
      ...
      if (response.data.status === true) {
        ...
        if (response.data.data.length > 0) {
          ...
          if (
            lastItem.hasOwnProperty("haveMore") ||
            lastItem.hasOwnProperty("totalCount")
          ) {
            metadata = {
              haveMore: lastItem.haveMore || false,
            ...
            userData = response.data.data;
            metadata.haveMore =
              followingData.length + userData.length < totalFollowing;
          }
        }
        ...
        if (pageNum === 1) {
          ...
          setFollowingData((prevData) => [...prevData, ...userData]);
        }

        if (metadata.totalCount > 0) {
          setTotalFollowing(metadata.totalCount);
        }
  ...
  const onRefresh = () => {
    ...
    setHasMore(true);
    setFollowingData([]);
    fetchFollowingData(user, 1, false);
  };

  const loadMore = () => {
    if (hasMore && !loadingMore && !refreshing) {
      fetchFollowingData(user, page + 1, false);
    }
  };

  const filteredFollowing = followingData.filter((item) => {
    ...
    if (loadingMore) {
      return (
        <View style={styles.footerLoader}>
          ...
          <Text style={styles.footerText}>Loading more followings...</Text>
        </View>
      );
    }

    if (!hasMore && followingData.length > 0) {
      return (
        <View style={styles.footerLoader}>
          <Text
            style={[styles.footerText, { color: "#333", fontWeight: "600" }]}
          >
            No more followings to load
          </Text>
        </View>
      );
    }
  ...
  const renderFollowingItem = ({ item, index }) => (
    <TouchableOpacity 
    ...
    >
      <View style={styles.userCard}>
        ...
        <View style={styles.userInfo}>
          {item.isFollowing ? (
            <Text style={styles.followerText}>Follows You</Text>
          ) : null}
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          {totalFollowing > 0 && (
            <Text style={styles.followerCount}>{totalFollowing}</Text>
          )}
        </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyStateTitle}>No such followings found</Text>
