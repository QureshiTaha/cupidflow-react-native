...
const ResetPasswordScreen = () => {
  ...
  const handleResetPassword = async () => {
    if (!password) {
      toastRef.current?.show("Please enter your new password", {
        type: "danger",
      });
      return;
    }
    ...
    if (password.length < 6) {
      toastRef.current?.show("Password must be at least 6 characters long", {
        type: "danger",
      });
      return;
    }
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          // Backend responded with status code (400, 401, etc)
          const message =
        ...
        // Unknown error
        toastRef.current?.show("❌ Something went wrong. Please try again.", {
          type: "danger",
        });
      }
    } finally {
