...
const ReelItem = ({ item, shouldPlay, openShareOptions, currentIndex, index }) => {
    // const { item, shouldPlay, openShareOptions } = props;
    ...
    const [showCommentsOptionsModal, setShowCommentsOptionsModal] = useState(false);
            ...
            try {
                ...
                if (user) {
                    ...
                    const response = await axios.get(
                        `${BASE_URL}/api/v1/reels/interaction-status/${item.reelId}/${parsedUser.userID}`
                    );
    ...
    const deleteReel = async () => {
        ...
        try {
            await axios.delete(
                `${BASE_URL}/api/v1/reels/delete/${item.reelId}`
            );
            setShowMoreOptionsModal(false);
    ...
    const sendComment = async (
        ...
        try {
            ...
            const response = await axios.post(
                `${BASE_URL}/api/v1/reels/add-comment`,
                data,
                {
                    headers: {
                        "Content-Type": "application/json",
    ...
    const handlePlaybackStatusUpdate = useCallback(
        debounce((status) => setStatus(status), 300),
        []
    );
    ...
    const videoUrl = useMemo(() => {
        return `${BASE_URL}/reels${item.filepath}`;
    }, [item.filepath]);
        ...
        <View style={styles.videoContainer}>
            <Pressable onPress={togglePlayback}>
                ...
                {/* <Video
                    ...
                    isLooping={true}
                    shouldPlay={shouldPlay} // controls playback
            ...
            <Modal animationType="slide"
                ...
                onRequestClose={() => setShowCommentModal(false)}showFetchedComments >
...
export default React.memo(ReelItem, (prev, next) => {
    return (
